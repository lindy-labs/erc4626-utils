 ...
 * The contract assumes that ERC4626 tokens are appreciating assets, meaning that the value of a single share increases over time, i.e., generates yield.
 */
contract YieldStreaming is StreamingBase {
     ...
     * When opening a new stream for a receiver which is currently in debt, the sender incurs an immediate loss by taking a share of the receiver's debt.
     * The debt is calculated as the difference between the current value of the receiver's streams and the total principal allocated to the receiver.
     ...
     * @param _maxOnOpenTolerancePercent The maximum loss tolerance percentage when opening a stream to a receiver which is in debt.
     * @return principal The amount of assets (tokens) allocated to the stream.
     */
    function openYieldStream(address _receiver, uint256 _shares, uint256 _maxOnOpenTolerancePercent)
        public
        returns (uint256 principal)
    {
        ...
        _checkImmediateLossOnOpen(_receiver, msg.sender, principal, _maxOnOpenTolerancePercent);
    ...
    function openYieldStreamUsingPermit(
        address _receiver,
        uint256 _shares,
        uint256 _maxOnOpenTolerancePercent,
        uint256 deadline,
        uint8 v,
        bytes32 r,
        bytes32 s
        ...
        principal = openYieldStream(_receiver, _shares, _maxOnOpenTolerancePercent);
    }

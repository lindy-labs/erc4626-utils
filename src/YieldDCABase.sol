...
/**
 * @title YieldDCABase
 * @notice Implements a base contract for a Dollar Cost Averaging (DCA) strategy using the yield generated by an ERC4626 vault.
 * @dev This contract provides the core functionalities for managing DCA positions, yield conversion, and position tracking. It supports the management of DCA positions represented by ERC721 tokens.
 *
 * ## Key Features
 ...
 * - **Yield Tracking:** Tracks the yield generated from user positions.
 * - **Configurable Parameters:** Admins can configure epoch duration, minimum yield per epoch, and discrepancy tolerance.
 * - **Role-Based Access Control:** Utilizes AccessControl for managing admin roles.
 *
 * ## Roles
 ...
 */
abstract contract YieldDCABase is ERC721, AccessControl, Multicall {
    ...
    */
    ...
    /// @dev redeems yield from the underlying vault. Only to be used inside epoch execution.
    function _redeemYield() internal returns (uint256 yield, uint256 yieldInShares) {
        _checkEpochDuration();

        // calculate yield in shares and redeem from the vault
        uint256 totalPrincipal_ = totalPrincipal;
        yieldInShares = _calculateYieldInShares(totalPrincipal_);
    ...
    /// @dev updates the epoch information and emits the {DCAExecuted} event. Only to be used inside epoch execution.
    function _updateEpoch(uint256 amountOut, uint256 yield, uint256 yieldInShares) internal {
